...
class IODeepAttemptBenchmark {
  ...
  @Benchmark
  def arrowsTwitterTaskDeepAttempt(): BigInt = {
    import arrows.twitter._
    import com.twitter.util.Await

    def descend(n: Int): Task[BigInt] =
      if (n == depth) Task.exception(new Error("Oh noes!"))
      else if (n == halfway) descend(n + 1).handle { case _ => 50 }
      else descend(n + 1).map(_ + n)

    Await.result(descend(0).run)
  }
