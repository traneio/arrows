...
private[arrows] final object ArrowAst {
  ...
  final case class Collect[T, U](seq: Seq[Arrow[T, U]]) extends Arrow[T, Seq[U]] {
    override final def runSync[B <: T](s: Sync[B], depth: Int)(implicit ec: ExecutionContext): Result[Seq[U]] =
      if (!s.success)
        s.as[Seq[U]]
      else {
        val v = s.value
        ...
        if (success) {
          ...
          Async(null, Future.sequence(arr.toSeq.asInstanceOf[Seq[Future[U]]]))
        }
      }
  }
